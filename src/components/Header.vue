<template>
  <div class="rounded-xl px-8 py-4 inline-block ">
    <div v-if="!store.userName || isEditing" class="flex items-center gap-3">
      ...
      />
      <h1 class="text-4xl font-bold text-green-200">Todo Forest ✓</h1>
    </div>
    
    <div v-else class="flex items-center gap-3">
      <h2 
        @click="editName"
        class="text-4xl font-bold text-green-100 cursor-pointer hover:text-green-700 hover:underline"
      >
        {{ store.userName }}'s
      </h2>
      <h1 class="text-4xl font-bold text-lime-200">Todo Forest ✓</h1>
    </div>
  </div>
</template>
